...
static const MoveEvaluator moveEvaluators[3] = {
...
int AttackerMonWithHighestDamage(struct BattleSystem *bsys, u32 battler, AIContext *ai);
...
int BasicFlag (struct BattleSystem *bsys, u32 attacker, int i, AIContext *ai)
{
   ...
   // if (ctx->clientPriority[ctx->attack_client] > 0 && GetBattlerAbility(ctx, ctx->attack_client) == ABILITY_PRANKSTER && HasType(ctx, defender, TYPE_DARK) && (ctx->attack_client & 1) != (defender & 1)) // used on an enemy)
    ...
    /*OHKO moves*/
    else if ((ai->attackerMoveEffect == MOVE_EFFECT_ONE_HIT_KO && (ai->defenderAbility == ABILITY_STURDY && ai->attackerAbility != ABILITY_MOLD_BREAKER)) || 
        ctx->battlemon[attacker].level < ctx->battlemon[ai->defender].level ||
        (ctx->battlemon[ai->defender].item == ITEM_FOCUS_SASH && ctx->battlemon[ai->defender].hp == ctx->battlemon[ai->defender].maxhp)) {
             moveScore -= NEVER_USE_MOVE_20;
    }
    ...
    if((ai->attackerMoveEffect == MOVE_EFFECT_FAINT_FULL_RESTORE_NEXT_MON) &&
        (ai->livingMembersAttacker == 1)){
        moveScore -= NEVER_USE_MOVE_20;//can penalize twice
    }
...
int EvaluateAttackFlag (struct BattleSystem *bsys, u32 attacker, int i, AIContext *ai)
{
        ...
        ai->attackerMove == MOVE_ROLLOUT || ai->attackerMove == MOVE_METEOR_BEAM || ai->attackerMove == MOVE_FINAL_GAMBIT)
    {
        ...
        {
            if(ai->defenderAbility != ABILITY_DAMP || ai->attackerAbility == ABILITY_MOLD_BREAKER)
            {
        ...
        if(ai->attackerMove == MOVE_FINAL_GAMBIT)
        {
            if (ai->attackerMovesFirst && ai->attackerHP > ai->defenderHP)
                moveScore += 8;
            else if (ai->attackerMovesFirst && ai->attackerHP < ai->maxDamageReceived)
                moveScore += 7;
            else
                moveScore += 6;
        }
    ...
    if (ai->attackerMaxDamageOutputMinRoll >= ai->defenderHP) // if ai sees kill with this move
    {
...
int HarassmentFlag(struct BattleSystem* bsys, u32 attacker, int i, AIContext* ai)
{
    ...
    switch (ai->attackerMoveEffect)
    {
        ...
        case MOVE_EFFECT_SET_REFLECT:
         ...
         case MOVE_EFFECT_STATUS_SLEEP_NEXT_TURN:
             moveScore += 6;
             if (ai->attackerMaxDamageOutputMinRoll < ai->defenderHP && (BattleRand(bsys) % 4 == 0)) // no kill
             {
                 if (!ai->defenderImmuneToSleep)
                     moveScore += 1;
         ...
         case MOVE_EFFECT_STATUS_BADLY_POISON:
             moveScore += 6;
             if (ai->attackerMaxDamageOutputMinRoll < ai->defenderHP && (BattleRand(bsys) % 3 == 0)) // no kill
             {
...
BOOL BattlerHasMoveSplit (struct BattleSystem *bsys, u32 battler, u32 move_split, AIContext *ai)
{
    BOOL hasMoveSplit = FALSE;
    struct BattleStruct *ctx = bsys->sp;

    for(int i = 0; i < CLIENT_MAX; i++)
    {   
        u32 moveno = ctx->battlemon[battler].move[i];
        if (moveno != MOVE_NONE)
        {
            struct BattleMove move = ctx->moveTbl[moveno];
            if(move.split == move_split)
            {
                hasMoveSplit = TRUE;
                break;
            }
...
void SetupStateVariables(struct BattleSystem *bsys, u32 attacker, u32 defender, AIContext *ai){
    ...
    for(int i = 0; i < ai->attackerMovesKnown; i++){
        ...
        if(move.split != SPLIT_STATUS)
        {
            ...
            debug_printf("move %d: %d, damage %d\n", i, attackerMoveCheck, ai->attackerMinRollMoveDamages[i]);
            ai->attackerMinRollMoveDamages[i] = AdjustUnusualMoveDamage(bsys, ai->attackerLevel, ai->attackerHP, ai->defenderHP, ai->attackerMinRollMoveDamages[i], attackerEffectCheck);
            debug_printf("move %d: %d, damage %d\n", i, attackerMoveCheck, ai->attackerMinRollMoveDamages[i]);
        }
